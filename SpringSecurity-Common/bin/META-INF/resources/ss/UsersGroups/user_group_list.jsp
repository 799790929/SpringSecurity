	...
	function relax(userid) {
		...
		//if(confirm("确认解除关系?")) {
			/*ajax.Req("get",url,"",function(o){
				...
				if(v == 'success') {
					alert("删除成功");
					window.location.href="${ctx}/ss/UsersGroups/doList.do";
				} else {
					alert(v);
				}
			});*/
		...
		//$("#doSearch").submit();
  		...
  		<%-- <div class="tags_bg">
	  		<c:forEach items="${groups}" var="item" varStatus="status">
	  			<div onclick="dogroups('${item.id}')" class="tags_but"  <c:choose><c:when test="${item.id==group.id }">  style="display:inline;background:#eee;color:#fff;"  </c:when><c:otherwise>  style="display:inline;" </c:otherwise></c:choose> >${item.name }</div>
	  		</c:forEach>
  		</div>
...
<!--           <div id="search_div"> -->
<%--             <c:forEach items="${groups}" var="item" varStatus="status"> --%>
<%-- 	  			<div  onclick="dogroups('${item.id}')" class="tags_but"  <c:choose><c:when test="${item.id==group.id }">  style="font-size: 16px;display:inline;background:#000;color:#fff;"  </c:when><c:otherwise>  style="font-size: 16px;display:inline;" </c:otherwise></c:choose> ><strong>${item.name }</strong></div> --%>
<%-- 	  		</c:forEach> --%>
<!--           </div> -->
          
         
          <div id="main_btn">
          	<%-- <a href="${ctx }/ss/UsersGroups/doSavePage.do?groupid=${group.id}" class="type_a"><div id="btn_2">+分配用户</div></a> --%>
          	<%-- <a href="javascript:addPage('${ctx }/ss/UsersGroups/doSavePage.do?groupid=${group.id}')" class="type_a"><div id="btn_2">+分配用户</div></a> --%>
