...
public class UsersGroupsAction extends BaseStruts2Action implements Preparable,ModelDriven{
	...
	public String initPage() {
		...
		if(null != groups && groups.size() > 0) {
			if(null == group)
				group = groups.get(0);
			String hql = "from cn.sf.ss.model.Users where id in (select userId from cn.sf.ss.model.UsersGroups where groupId='" + group.getId() + "')";
	...
	public String doSavePagddde() {
		...
		if(null != groups && groups.size() > 0) {
			if(null == group) 
				group = groups.get(0);
			String hql = "from cn.sf.ss.model.Users where id not in (select userId from cn.sf.ss.model.UsersGroups where groupId='" + group.getId() + "')";
			users = usersManager.getEntityDao().executeFind(hql, null);
		}
...
//		return indexPage;
	...
	public String doList() {
		...
		if(null != groups && groups.size() > 0) {
			...
			String hql = "from cn.sf.ss.model.Users where "+Users.u_pk_user+" in (select userId from cn.sf.ss.model.UsersGroups where groupId='" + group.getId() + "')";
			users = usersManager.getEntityDao().executeFind(hql, null);
		}
