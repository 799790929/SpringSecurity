...
import cn.sf.ss.model.Resources;
import cn.sf.ss.model.Roles;
import cn.sf.ss.model.RolesResources;
import cn.sf.ss.service.ResourcesManager;
import cn.sf.ss.service.RolesManager;
import cn.sf.ss.service.RolesResourcesManager;
import cn.sf.ss.vo.query.RolesResourcesQuery;
...
public class RolesResourcesAction extends BaseStruts2Action implements Preparable,ModelDriven{
	...
	public String initPage() {
		...
		if(null != roles && roles.size() > 0) {
			if(null == role)
				role = roles.get(0);
			String hql = "from cn.sf.ss.model.Resources where id in (select resourceId from cn.sf.ss.model.RolesResources where roleId='" + role.getId() + "')";
	...
	public String doSavePddage() {
		...
		if(null != roles && roles.size() > 0) {
			if(null == role)
				role = roles.get(0);
			String hql = "from cn.sf.ss.model.Resources where id not in (select resourceId from cn.sf.ss.model.RolesResources where roleId='" + role.getId() + "')";
			resources = resourcesManager.getEntityDao().executeFind(hql, null);
		}
	...
	private cn.sf.ss.security.MyInvocationSecurityMetadataSource mySecurityMetadataSource;
	
	public cn.sf.ss.security.MyInvocationSecurityMetadataSource getMySecurityMetadataSource() {
		return mySecurityMetadataSource;
	}

	public void setMySecurityMetadataSource(
			cn.sf.ss.security.MyInvocationSecurityMetadataSource mySecurityMetadataSource) {
	...
	public String doList() {
		...
		if(null != roles && roles.size() > 0) {
			if(null == role)
				role = roles.get(0);
			String hql = "from cn.sf.ss.model.Resources where "+ Resources.rs_pk_resource +" in (select resourceId from cn.sf.ss.model.RolesResources where roleId='" + role.getId() + "')";
			resources = resourcesManager.getEntityDao().executeFind(hql, null);
		}
...
//		return "/myIndex.jsp";
	...
	public String doSavePage() {
		...
		if(null != roles && roles.size() > 0) {
			if(null == role)
				role = roles.get(0);
			String hql = "from cn.sf.ss.model.Resources where PK_RESOURCE not in (select resourceId from cn.sf.ss.model.RolesResources where roleId='" + role.getId() + "')";
			resources = resourcesManager.getEntityDao().executeFind(hql, null);
		}*/
